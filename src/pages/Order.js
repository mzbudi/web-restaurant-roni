...
class Order extends React.Component{
    constructor(props) {
        super(props)
        this.state = {
            label : ['Jan', 'Feb', 'Mar', 'Apr', 'Mei', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Des'],
            total : []
    ...
    render(){
        ...
        if(this.props.order.isLoading){
            this.props.order.data.data.data.map((data,i)=>{
                totalOrders += 1
                if(String(Moment(data.created_at).format('DD/MM/YYYY')) === String(today)){
                    arrToday.push(data.subTotal)
                }
                totalMonth.forEach((month)=>{
                    if(parseInt(Moment(data.created_at).format('DD/MM/YYYY').substring(3,5)) == month){
        ...
        return(
            ...
            <NavbarNavigation />
            <Container>
            <CardHistory totalOrders={totalOrders} todayTotals={arrToday} totalRevenue={totalRevenue}/>
            <Chart totalRevenue={totalRevenue} />
            <h4 style={{textAlign: "center", marginTop: "80px"}}> Recent Orders</h4>
            <Table>
                <thead>
                    <tr>
                        <th>No</th>
                    ...
                    this.props.order.data.data.data.map((data,i)=>{
                        ...
                        return(
                            <tr key={i}>
                                ...
                                <td>{data.subTotal}</td>
                                <td>{data.PPn}</td>
                                <td style={{textAlign: "center"}}>{Moment(data.created_at).format('DD/MM/YYYY')}</td>
